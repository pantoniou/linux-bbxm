...
bone_capemgr_add_slot(struct bone_capemgr_info *info,
		struct device_node *node);
		 ...
		 const char *buf, size_t count)
{
	...
	printk(KERN_INFO "override part_number '%s', version '%s'\n",
...
static ssize_t dt_tree_store(struct device *dev, struct device_attribute *attr,
		 const char *buf, size_t count)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct bone_capemgr_info	*info = platform_get_drvdata(pdev);
...
static DEVICE_ATTR(dt_tree, 0222, NULL, dt_tree_store);
...
static int bone_capemgr_info_sysfs_register(struct bone_capemgr_info *info)
{
	...
	ret = device_create_file(dev, &dev_attr_dt_tree);
	if (ret != 0)
		goto err_fail_no_dt_tree;
...
err_fail_no_bbrd_grp:
	device_remove_file(dev, &dev_attr_dt_tree);
err_fail_no_dt_tree:
...
bone_capemgr_add_slot(struct bone_capemgr_info *info, struct device_node *node)
{
	...
	if (!of_property_read_bool(node, "ti,cape-override")) {
		...
		ret = bone_slot_fill_override(slot, node);
		if (ret != 0) {
