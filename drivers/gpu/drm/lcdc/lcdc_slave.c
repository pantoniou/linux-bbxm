...
static void slave_encoder_prepare(struct drm_encoder *encoder)
{
	...
	lcdc_crtc_set_panel_info(encoder->crtc, &slave_info);
}
...
/* TODO probably should get this from DT.. */
		  ...
		  struct drm_display_mode *mode)
{
	...
	ret = lcdc_crtc_mode_valid(priv->crtc, mode);
	if (ret != MODE_OK)
		return ret;
...
static int slave_modeset_init(struct lcdc_module *mod, struct drm_device *dev)
{
...
static void slave_destroy(struct lcdc_module *mod)
{
	struct slave_module *slave_mod = to_slave_module(mod);
...
static const struct lcdc_module_ops slave_module_ops = {
...
static int slave_probe(struct platform_device *pdev)
{
	...
	if (!of_match_device(slave_of_match, &pdev->dev)) {
	...
	lcdc_module_init(mod, "slave", &slave_module_ops);
...
int __init lcdc_slave_init(void)
{
	return platform_driver_register(&slave_driver);
}

void __exit lcdc_slave_fini(void)
{
