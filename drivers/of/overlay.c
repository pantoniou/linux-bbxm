	...
	__for_each_child_of_node(overlay, child) {
		...
		if (tchild != NULL) {
			...
			if (!remove) {
				...
				/* NOTE: Don't free tchild; it's in the log */
			}
...
struct of_overlay_record_entry *of_overlay_record_entry_lookup(
		struct of_overlay_info *ovinfo, struct device_node *node)
{
	struct of_overlay_record_entry *re;

	/* no need for locks, we're under the ovinfo->lock */
	list_for_each_entry(re, &ovinfo->re_list, node) {
		if (re->np == node)
			return re;
	}
	return NULL;
...
static void of_overlay_record_entry_entry_add(struct of_overlay_info *ovinfo,
		...
		int state)
{
	...
	fresh = 0;
	re = of_overlay_record_entry_lookup(ovinfo, node);
	if (re == NULL) {
	...
	if (fresh)
		list_add_tail(&re->node, &ovinfo->re_list);
}
				...
				unsigned long action, void *arg)
{
	...
	/* come up with the record entry (if any) */
	pdev = NULL;
	state = 0;
	...
	of_overlay_record_entry_entry_add(ovinfo, node, pdev, state);
...
{
	struct of_overlay_record_entry *re, *ren;
	...
	list_for_each_entry_safe_reverse(re, ren, &ovinfo->re_list, node) {
...
static int of_overlay_post_one(struct of_overlay_info *ovinfo, int err)
{
	...
	/* overlay applied correctly, now create/destroy pdevs */
	list_for_each_entry_safe(re, ren, &ovinfo->re_list, node) {
