...
phandle __of_get_tree_max_phandle(struct device_node *node, phandle max_phandle)
{
		...
		int phandle_delta)
{
	...
	__for_each_property_of_node(child, rprop) {
		...
		for (propcur = propval; propcur < propend;
			...
			*s++ = '\0';
			...
			if (refnode == NULL) {
				pr_warn("%s: Could not find refnode '%s' (gone during versioning?)\n",
					__func__, (char *)rprop->value);
				continue;
			}

			/* now find the property */
			__for_each_property_of_node(refnode, sprop) {
			...
			*(uint32_t *)(sprop->value + offset) =
			...
			/* pr_info("%s: \t\tnode=%s prop=%s "
					"offset=%d;\n", __func__,
					nodestr, propstr, offset); */
		}
...
int of_resolve(struct device_node *resolve)
{
	...
	__for_each_property_of_node(resolve_fix, rprop) {
		...
		if (!of_is_printable_string(rprop->value, rprop->length))
			continue;
